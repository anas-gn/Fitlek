...
import { sirvyaLogin as makeSirvyaLoginHandler } from './jwt_bridge.js';
...
const routes = {
  ...
  // Sirvya SSO: verify a Firebase ID token from the SIRVYA mobile app (or its embedded
  // workout WebView), map it to an openGym account (auto-created on first sign-in via the
  // Firebase uid as `sirvyaUid`), and return a session in the same signed format the rest
  // of this server uses. The returned `token` is a bearer token the WebView's api() sends
  // as `Authorization: Bearer ...`, which readSession() accepts just like the cookie.
  // Lazily initializes firebase-admin; when `FIREBASE_SERVICE_ACCOUNT` (Sirvya's
  // project) is not set this responds 503 so passkey-only instances are unaffected.
  'POST /api/auth/sirvya-login': makeSirvyaLoginHandler(db, saveDb, json, readBody, audit),
