...
class _CoachDashboardState extends State<CoachDashboard> {
  ...
  List<Map<String, dynamic>> _recentClientInvitations = [];
    ...
    if (result['ok'] == true) {
      setState(() {
        ...
        _notifications = ((result['notifications'] as List?) ?? [])
            ...
            .map((e) => Map<String, dynamic>.from(e))
            .toList();
        _recentClientInvitations =
            ((result['recentClientInvitations'] as List?) ?? [])
                .whereType<Map>()
                .map((e) => Map<String, dynamic>.from(e))
                .toList();
  ...
  Widget _buildClientInvitations(ColorScheme cs, FitlekColors f) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _sectionHeader(
          'Client Invitations',
          action: widget.onViewInvitations == null ? null : 'View all',
          onAction: widget.onViewInvitations,
          cs: cs,
        ),
        const SizedBox(height: 12),
        if (_recentClientInvitations.isEmpty)
          _emptyCard(
            icon: Icons.mark_email_unread_outlined,
            title: 'No client invitations',
            subtitle: 'Invitations sent by clients will appear here.',
            f: f,
          )
        else
          ..._recentClientInvitations.map(
            (item) => _ClientInvitationTile(
              item: item,
              onTap: widget.onViewInvitations,
            ),
          ),
      ],
    );
...
class _ClientInvitationTile extends StatelessWidget {
  final Map<String, dynamic> item;
  final VoidCallback? onTap;

  const _ClientInvitationTile({required this.item, this.onTap});

  String _timeAgo() {
    final date =
        DateTime.tryParse(item['clickedAt']?.toString() ?? '')?.toLocal();
    if (date == null) return '';
  ...
  Widget build(BuildContext context) {
    ...
    final avatar = item['avatarUrl']?.toString();
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: pending
                ? f.warning.withValues(alpha: 0.28)
                : f.border,
          ),
        ),
        child: Row(
