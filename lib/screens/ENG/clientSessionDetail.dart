...
class _SessionDetailScreenState extends State<SessionDetailScreen>
  ...
  Color get _statusColor {
    if (_session.isConfirmed) return context.fitlek.success;
    if (_session.isPending) return Theme.of(context).colorScheme.primary;
    return context.fitlek.error;
  }

  String get _statusLabel {
    if (_session.isPending) return 'PENDING';
    if (_session.isConfirmed) return 'CONFIRMED';
    return 'CANCELLED';
  }
  ...
  Widget _buildCancelledInfo() {
    final reason =
        _session.cancellationReason ?? _session.rejectionReason ?? '';
