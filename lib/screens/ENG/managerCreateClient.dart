...
class ManagerCreateClient extends StatefulWidget {
  const ManagerCreateClient({super.key});
  @override
  State<ManagerCreateClient> createState() => _ManagerCreateClientState();
}

class _ManagerCreateClientState extends State<ManagerCreateClient> {
  ...
  void _save() async {
    ...
    final result = await ApiService.post('/manager/clients/create', {
    ...
    if (result['ok'] == true) {
      ...
      ApiService.showError(context, result['message'] ?? 'Create failed.');
    }
  }
  ...
  Widget _topBar() => Padding(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 12),
    child: Row(children: [
      ...
      const Text('Create Client', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w700)),
    ]));
  ...
  Widget _submitBtn() => GestureDetector(onTap: _loading ? null : _save,
    child: Container(width: double.infinity, height: 56,
      ...
      child: Center(child: _loading
        ...
        : const Text('Create Client', style: TextStyle(color: Colors.black, fontSize: 16, fontWeight: FontWeight.w800)))));
}
