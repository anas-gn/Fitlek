...
class _CoachInvitationsState extends State<CoachInvitations> {
  ...
  Future<void> _load() async {
    ...
    if (result['ok'] == true) {
      ...
      setState(() {
        _invitations = list;
        _totalPoints = list.where((inv) => inv['status'] == 'accepted').fold(0, (sum, inv) => sum + ((inv['pointsEarned'] as num?)?.toInt() ?? 0));
        _loading = false;
      });
    } else {
  ...
  Future<void> _respond(dynamic inv, bool accept) async {
    ...
    if (result['ok'] == true) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(result['message']?.toString() ?? 'Erreur, réessayez.')),
      );
    }
  }
      ...
      Row(children: [
        ...
        if (inv['status'] != 'pending')
          Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
            decoration: BoxDecoration(
              color: inv['status'] == 'accepted' ? const Color(0xFF1A3008) : const Color(0xFF2A1010),
              borderRadius: BorderRadius.circular(10),
              border: Border.all(color: inv['status'] == 'accepted' ? const Color(0xFFA3FF12).withOpacity(0.3) : Colors.redAccent.withOpacity(0.3))),
            child: Text(
              inv['status'] == 'accepted' ? '+${inv['pointsEarned']} pts' : 'Refusée',
              style: TextStyle(color: inv['status'] == 'accepted' ? const Color(0xFFA3FF12) : Colors.redAccent, fontSize: 13, fontWeight: FontWeight.w700))),
      ]),
      if (inv['status'] == 'pending') ...[
        const SizedBox(height: 12),
        Row(children: [
          Expanded(child: OutlinedButton(
            ...
            child: const Text('Refuser', style: TextStyle(color: Colors.white70, fontWeight: FontWeight.w600)))),
          const SizedBox(width: 10),
          Expanded(child: ElevatedButton(
            onPressed: () => _respond(inv, true),
            style: ElevatedButton.styleFrom(
              backgroundColor: const Color(0xFFA3FF12),
              padding: const EdgeInsets.symmetric(vertical: 10),
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
            child: const Text('Accepter', style: TextStyle(color: Colors.black, fontWeight: FontWeight.w700)))),
        ]),
      ],
    ]),
  );
