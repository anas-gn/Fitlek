...
class _LoginScreenState extends State<LoginScreen>
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (role == 'client') {
          ...
          setState(() {
            _errorMsg = 'Ce compte ($role) n\'a pas accès à l\'app mobile.';
            _loading = false;
          });
          return;
        }
        ...
        setState(() {
          _errorMsg = _friendly(body['error'] as String? ?? 'Erreur.');
          _loading = false;
        });
      }
    } catch (_) {
  ...
  String _friendly(String raw) {
    ...
    if (raw.contains('banned')) return 'Ce compte est suspendu.';
    if (raw.contains('required')) return 'Veuillez remplir tous les champs.';
    return raw;
  }
  ...
  Widget _buildHeading() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 12),
      Text(
        'Connectez-vous pour accéder à votre espace personnel et gérer vos séances.',
        style: TextStyle(
          fontSize: 14,
          height: 1.5,
