  ...
  try {
    ...
    const [rows] = await db.query(
      `SELECT COUNT(*) AS totalUnread
       ...
       WHERE (c.clientID = ? OR c.coachID = ?) AND m.senderID != ? AND m.isRead = 0`,
  ...
  try {
    ...
    if (role === 'client') {
      sql = `SELECT cv.*, CONCAT(co.firstName,' ',co.lastName) AS otherName, co.avatarUrl AS otherAvatar,
             ...
             WHERE cv.clientID=? 
               AND co.id NOT IN (SELECT blockedID FROM user_blocks WHERE blockerID = ?)
               AND co.id NOT IN (SELECT blockerID FROM user_blocks WHERE blockedID = ?)
             ORDER BY cv.lastMessageAt DESC`;
      ...
      sql = `SELECT cv.*, CONCAT(c.firstName,' ',c.lastName) AS otherName, c.avatarUrl AS otherAvatar,
             ...
             WHERE cv.coachID=? 
               AND c.id NOT IN (SELECT blockedID FROM user_blocks WHERE blockerID = ?)
               AND c.id NOT IN (SELECT blockerID FROM user_blocks WHERE blockedID = ?)
             ORDER BY cv.lastMessageAt DESC`;
      params = [userID, requesterID, requesterID];
    } else {
  ...
  try {
    ...
    const [rows] = await db.query(
      `SELECT * FROM conversations 
       WHERE clientID=? AND coachID=? 
         AND clientID NOT IN (SELECT blockedID FROM user_blocks WHERE blockerID = ?)
         AND clientID NOT IN (SELECT blockerID FROM user_blocks WHERE blockedID = ?)
         AND coachID NOT IN (SELECT blockedID FROM user_blocks WHERE blockerID = ?)
         AND coachID NOT IN (SELECT blockerID FROM user_blocks WHERE blockedID = ?)
       LIMIT 1`,
      [clientID, coachID, requesterID, requesterID, requesterID, requesterID]
    );
