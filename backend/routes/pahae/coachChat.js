...
router.post('/:conversationId', requireAuth, requireRole('coach'), async (req, res) => {
  ...
  if ((!body || !body.trim()) && !mediaUrl) return res.status(400).json({ message: 'Message content is required.' });
  try {
    const [[conv]] = await pool.query(
    ...
    const [result] = await pool.query(
      ...
      [conversationId, coachID, body ? body.trim() : null, mediaUrl || null, mediaType || 'text']
    );
    await pool.query(
    ...
    res.status(201).json({ id: result.insertId, body: body ? body.trim() : null, mediaUrl, mediaType, senderID: coachID });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
});
