...
// Idempotently ensures the deleted_accounts audit table exists.
// Stores the minimum PII-free data needed for anti-fraud purposes after account deletion.
// Complies with GDPR / App Store / Play Store account deletion requirements.
export async function ensureDeletedAccountsSchema() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS deleted_accounts (
      id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
      original_user_id BIGINT UNSIGNED NOT NULL,
      email_hash VARCHAR(64) NOT NULL COMMENT 'SHA-256 of email, not reversible',
      role ENUM('client','coach','advisor','manager','admin') NOT NULL DEFAULT 'client',
      deleted_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
      deletion_ip VARCHAR(64) NULL,
      PRIMARY KEY (id),
      KEY idx_deleted_email_hash (email_hash),
      KEY idx_deleted_at (deleted_at)
    )
  `);
}

// Idempotently ensures the app_versions table exists.
export async function ensureAppVersionSchema() {
  await pool.query(`
  ...
  await pool.query(`
    INSERT IGNORE INTO app_versions (platform, latest_version, min_required_version, store_url)
    VALUES 
      ('ios', '1.0.0', '1.0.0', 'https://apps.apple.com/app/idYOUR_APP_ID'),
      ('android', '1.0.0', '1.0.0', 'https://play.google.com/store/apps/details?id=com.fitlek.app')
  `);
}

// Idempotently ensures UGC compliance tables exist (App Store / Play Store policy).
export async function ensureUgcComplianceSchema() {
  await pool.query(`
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS user_reports (
      ...
      type ENUM('user', 'message') NOT NULL DEFAULT 'user',
      entityID BIGINT UNSIGNED NULL COMMENT 'ID of the specific message if type is message',
      createdAt TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
      PRIMARY KEY (id),
      KEY idx_reporter (reporterID),
      KEY idx_reported (reportedID),
      CONSTRAINT fk_reporter FOREIGN KEY (reporterID) REFERENCES users(id) ON DELETE CASCADE,
      CONSTRAINT fk_reported FOREIGN KEY (reportedID) REFERENCES users(id) ON DELETE CASCADE
    )
  `);
}

// Idempotently ensures the coachimages table exists.
export async function ensureCoachImagesSchema() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS coachimages (
      id INT AUTO_INCREMENT PRIMARY KEY,
      coachID BIGINT UNSIGNED NOT NULL,
      urlImage VARCHAR(600) NOT NULL,
      createdAt TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
      KEY idx_coachimages_coachID (coachID),
      CONSTRAINT fk_coachimages_coachID FOREIGN KEY (coachID) REFERENCES users(id) ON DELETE CASCADE
    )
  `);
}
